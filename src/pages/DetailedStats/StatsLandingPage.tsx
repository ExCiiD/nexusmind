...
function SeriesFilterBar({ selected, onChange }: { selected: string[]; onChange: (next: string[]) => void }) {
  const { t } = useTranslation()
  ...
  const toggleCategory = (cat: string) => {
    ...
    if (allActive) onChange(selected.filter((k) => !catKeys.includes(k)))
    ...
    <div className="space-y-2">
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <button onClick={toggleAll}
          className={cn('rounded-full border px-3 py-0.5 text-[10px] font-medium transition-colors',
            isAll ? 'border-hextech-gold bg-hextech-gold/10 text-hextech-gold-bright' : 'border-hextech-border-dim text-hextech-text-dim hover:border-hextech-border')}
        >
        ...
        {LINE_CATEGORIES.map((cat) => {
          ...
          return (
            <button key={cat} onClick={() => toggleCategory(cat)}
              className={cn('rounded-full border px-3 py-0.5 text-[10px] font-medium transition-colors capitalize',
                allActive ? 'border-hextech-gold bg-hextech-gold/10 text-hextech-gold-bright' : 'border-hextech-border-dim text-hextech-text-dim hover:border-hextech-border')}
            >
              {cat}
            </button>
          )
      ...
      <div className="flex items-center gap-1.5 flex-wrap">
        {LINE_SERIES.map((s) => (
          <button key={s.key} onClick={() => toggleSeries(s.key)}
            className={cn('rounded-full border px-2.5 py-0.5 text-[10px] font-medium transition-colors',
              selected.includes(s.key) ? 'border-hextech-gold/60 text-hextech-text-bright' : 'border-hextech-border-dim text-hextech-text-dim hover:border-hextech-border')}
    ...
    return (
      ...
      >
        ...
        <span className="ml-1 opacity-60">
          {new Date(s.firstGameAt).toLocaleDateString(undefined, { day: 'numeric', month: 'short' })}
          {' → '}
  ...
  return (
    <div className="space-y-4">
      <div className="space-y-2">
        ...
        {hasMore && (
          <Button variant="ghost" size="sm" className="text-xs gap-1 text-hextech-text-dim"
      ...
      <div className="grid grid-cols-4 gap-2 text-xs font-medium text-hextech-text-dim px-3 py-2 bg-hextech-elevated rounded-lg">
        ...
        <span className="text-right text-hextech-cyan">
          {t('statsAvg.progTable.snapshot', { n: baseIdx + 1 })}
          <span className="ml-1 opacity-60 font-normal">
            avg of {base.gameCount} games
          </span>
        </span>
        <span className="text-right text-hextech-gold">
          {t('statsAvg.progTable.snapshot', { n: targetIdx + 1 })}
          <span className="ml-1 opacity-60 font-normal">
            avg of {target.gameCount} games
          </span>
        </span>
          ...
          {STAT_ROWS.map((row) => {
            ...
            return (
              <div key={row.label} className="grid grid-cols-4 gap-2 px-4 py-2 text-sm items-center hover:bg-hextech-elevated/50 transition-colors">
                <span className="text-xs text-hextech-text">{row.label}</span>
                <span className="text-right text-xs text-hextech-text-dim">{baseVal != null ? fmt(baseVal) : '—'}</span>
                <span className="text-right text-xs font-medium text-hextech-text-bright">{targetVal != null ? fmt(targetVal) : '—'}</span>
                <span className={cn(
