...
export function ShareSessionButton({ data, className }: Props) {
  ...
  const { buildEmbeds, buildText } = useShareSession()
  ...
  const sendToWebhook = async (wh: DiscordWebhook) => {
    ...
    try {
      ...
      await window.api.sendToDiscord(embeds, wh.url)
      toast({ title: `📨 Session sent to "${wh.name}"`, variant: 'gold' })
    } catch (err: any) {
      toast({
